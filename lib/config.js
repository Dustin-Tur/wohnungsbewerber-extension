...
(function (root) {
  ...
  WBA.CONFIG = {
    ...
    /** EXT-11: Zufallspause vor dem Sprung zur nächsten Anzeige eines
     *  Durchlaufs (ms) – entschärft das Serien-Aufruf-Muster der Bot-Erkennung. */
    RUN_PAUSE_MIN_MS: 800,
    RUN_PAUSE_MAX_MS: 2000,
  ...
  /* QUA-08/SEC-06: EINE esc()-Fassung für alle Kontexte (vorher zwei identische
     Kopien in content.js und dashboard.js). Escapt auch Quotes – heute stehen
     alle dynamischen Werte in Textknoten, aber ein künftiges `attr="${esc(x)}"`
     wäre sonst ein Attribut-Breakout. */
  WBA.esc = function (s) { return (s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;"); };
