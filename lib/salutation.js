...
(function (root) {
  ...
  function badge(cls) {
    const c = cls && cls.category;
    const t = cls && cls.titles && cls.titles.length ? cls.titles.join(" ") + " " : "";
    if (c === "frau") return { ok: true, text: "erkannt: Frau " + t + cls.name };
    if (c === "herr") return { ok: true, text: "erkannt: Herr " + t + cls.name };
    if (c === "familie") return { ok: true, text: "erkannt: Familie " + cls.name };
    if (c === "vorname") return { ok: true, text: "erkannt: " + cls.vorname + " (locker)" };
    if (c === "firma") return { ok: true, text: "Firma erkannt – neutrale Anrede" };
    ...
    if (cls && cls.name) {
      ...
      return { ok: false, text: "unsicher: „" + full + "“ – Anrede bitte wählen" };
    }
    return { ok: false, text: "neutral, da unsicher" };
  }
