...
(function (root) {
  ...
  const PORTALS = [
    ...
    },
    {
      ...
      buildSearchUrl(f) {
        ...
        const max = n(f.preisMax);
        if (max) parts.push("preis::" + max);
    ...
    },
    {
      ...
      // Titel: Die <h1> trägt die Kennung „expose-title" seit einem Umbau als data-qa
      // statt als id (14.08.2026 auf zwei Kölner Anzeigen nachgemessen – h1#expose-title
      // 0 Treffer, h1[data-qa="expose-title"] genau 1). Die alte id bleibt als Auffangnetz
      // stehen: sie kostet nichts und deckt einen noch nicht überall ausgerollten Umbau ab.
      contentSel: 'h1[data-qa="expose-title"], h1#expose-title, [class*="is24qa-"]',
    ...
    {
      id: "immowelt",
      // Immonet wurde von Aviv abgeschaltet: immonet.de antwortet nur noch mit
      // „301 → www.immowelt.de#x-sunset-redirect=imn" (geprüft 12.08.2026). Es kann
      // dort also keine Anzeige mehr geöffnet werden – Name, Host-Muster und die
      // Rechteanforderung im Manifest führen Immonet deshalb nicht mehr.
      name: "Immowelt",
      host: /(^|\.)immowelt\.de$/i,
      experimental: false,
      ...
      listingUrlRe: /immowelt\.de\/expose\//i,
      resultsUrlRe: /immowelt\.de\/(liste|suche)\//i,
      scrapeSel: 'a[href*="/expose/"]',
  ...
  // Match-Patterns der unterstützten Portale für chrome.tabs.query (EXT-02):
  // MUSS mit manifest.json (host_permissions / content_scripts.matches)
  // übereinstimmen – bei Portal-Änderungen beide Stellen pflegen.
  const URL_PATTERNS = [
  ...
  const dom = {
    listingId(portal, url) {
      ...
      // zuvor) und Immowelt (ALPHANUMERISCHE IDs, die der Ziffern-Fallback
    ...
    fillProfileFields(portal, doc, p) {
      ...
      // SEC-01: Ohne umschließendes <form> NICHT das ganze Dokument befüllen –
      // Scope ist dann die unmittelbare Umgebung des Nachrichtenfelds. Eine
      // präparierte Seite könnte sonst irgendwo versteckte Felder mit passenden
      // Platzhaltern auslegen und die kompletten Kontaktdaten abgreifen.
      const scope = form || (msgBox && msgBox.closest("section, article, div")) || null;
      if (!scope) return;
      const setVal = (el, val) => {
      ...
      const candidates = [...scope.querySelectorAll('input, textarea, select')].filter((e) => {
        ...
        // SEC-01: Unsichtbare Felder sind nie legitime Kontaktfelder – wohl aber
        // das klassische Muster, um Autofill-Daten unbemerkt abzugreifen.
    ...
    async driveSearchForm(portal, doc, filters) {
      ...
      // 4) Absende-Weg nur ERMITTELN, nicht ausführen (EXT-10, Variante B):
      // Die Suchmaske wird NIE selbsttätig abgeschickt. Der Aufrufer bekommt
      // eine submit()-Funktion und ruft sie erst nach ausdrücklicher
      // Nutzer-Bestätigung im Overlay auf – „Werkzeug, kein Bot".
      const doSubmit = () => {
  ...
  WBA.portals = { PORTALS, URL_PATTERNS, byId, forUrl, slug, dom };
