...
(function (root) {
  ...
  const KEYS = {
    ...
    selfcheck: "wba_selfcheck", // QUA-06: je Portal Fehlschlag-Zähler „Anzeigenseite ohne Kontaktformular"
  ...
  /**
   * LEG-04: Profil-Sicht für ANSCHREIBEN (Vorlage und KI). Einkommen und
   * Beschäftigungsstatus sind rechtlich vor Vertragsschluss meist nicht
   * geschuldet – wer sie nicht offenbaren will, schaltet sie im Profil ab.
   * Gespeichert wird die Sperre (hideIncome/hideEmployment), nicht die Erlaubnis:
   * Bestandsprofile ohne diese Felder verhalten sich damit unverändert (= nennen).
   *
   * Bewusst NICHT gefiltert: die Mieterselbstauskunft-PDF (ein Formular, das der
   * Vermieter ausdrücklich verlangt) und die Unterlagen-Checkliste (sie leitet aus
   * der Beschäftigung ab, welche Nachweise nötig sind). Der Schalter verspricht
   * „nicht im Anschreiben" – nicht „nirgends".
   * @param {Object} p Profil aus dem Speicher bzw. dem Formular.
   * @returns {Object} Kopie ohne die gesperrten Felder (Original bleibt unberührt).
   */
  function letterProfile(p) {
    const out = Object.assign({}, p || {});
    // Die Sperre gilt für den ganzen Haushalt: Wer sein Einkommen nicht nennen
    // will, will auch das der zweiten Person nicht im Brief stehen haben –
    // sonst wäre die Summe aus beiden ein Umweg um die eigene Entscheidung.
    if (out.hideIncome) { out.income = ""; out.p2Income = ""; }
    if (out.hideEmployment) { out.employment = ""; out.p2Employment = ""; }
    return out;
  }
  ...
  /**
   * QUA-06: Lokaler Selbsttest gegen Portal-Umbauten – OHNE Telemetrie.
   * Auf einer per URL-Regex erkannten Anzeigenseite MUSS ein Kontaktformular
   * auffindbar sein. found=false zählt hoch, found=true setzt zurück; das
   * Dashboard warnt ab CONFIG.SELFCHECK_WARN_FROM Fehlschlägen in Folge.
   * @param {string} portalId @param {boolean} found
   */
  async function getSelfcheck() { return (await get({ [KEYS.selfcheck]: {} }))[KEYS.selfcheck] || {}; }
  function bumpSelfcheck(portalId, found) {
    return withWriteLock(async () => {
      const sc = await getSelfcheck();
      const cur = sc[portalId] || { misses: 0 };
      sc[portalId] = found
        ? { misses: 0, lastOkTs: Date.now() }
        : { misses: (cur.misses || 0) + 1, lastMissTs: Date.now(), lastOkTs: cur.lastOkTs };
      await set({ [KEYS.selfcheck]: sc });
      return sc[portalId];
    });
  }

  /**
   * LEG-13: Sammel-Aufräumen der Bewerbungsliste – entfernt Einträge, die älter
   * als cutoffTs sind UND keinen der Schutz-Status tragen (aktive Kontakte mit
   * Antwort/Besichtigung bleiben immer). Läuft unter dem Schreib-Mutex.
   * @param {number} cutoffTs @param {string[]} keepStatuses
   * @returns {Promise<number>} Anzahl entfernter Einträge.
   */
  function cleanupTracker(cutoffTs, keepStatuses) {
    return withWriteLock(async () => {
      const list = await getTracker();
      const keep = list.filter((e) => (e.ts || 0) >= cutoffTs || (keepStatuses || []).indexOf(e.status) >= 0);
      const removed = list.length - keep.length;
      if (removed) await setTracker(keep);
      return removed;
    });
  }
  ...
  async function migrate() {
    if (!hasChrome) return;
    // LEG-13: wba_lasturl war ein toter personenbezogener Rest (zuletzt besuchte
    // URL, nie gelesen) – bei jedem Start idempotent entsorgen, auch die alte
    // localStorage-Kopie. Läuft VOR dem Migrations-Flag, erreicht also auch
    // längst migrierte Installationen.
    try { await remove("wba_lasturl"); } catch (e) { (WBA.log || console).warn("lasturl-Cleanup:", e); }
    try { if (typeof localStorage !== "undefined") localStorage.removeItem("wba_lasturl"); } catch (e) { (WBA.log || console).warn("lasturl-localStorage:", e); }
  ...
  WBA.store = {
    KEYS,
    get, set, remove,
    getProfile, setProfile, letterProfile,
    getFlat, setFlat,
    getTheme, setTheme,
    getLang, setLang,
    ...
    getSelfcheck, bumpSelfcheck,
    trackerKey, upsertTracker, removeTracker, cleanupTracker, hasApplied, APPLIED_STATUS,
    migrate,
  };
